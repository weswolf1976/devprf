...
p.filename = sprintf('./data/%02.f_r%02.f_RF_bar_multSize_%s.mat',p.subj,p.run,datestr(now,30));
if ~exist('./data','dir')
    mkdir('./data');
end
...
if p.scanner == 0
    p.resolution = [2560 1440]; % desired resolution, compare to the 'actual' resolution below
    p.refresh_rate = 60;
...
p.n_segments  = 1; % for now
p.dot_speed   = 15; % deg/s was
p.dot_life_sec= 0.05*3;% seconds
...
p.dot_color   = [0 0 255]*255;
p.segment_gap = 0.25*3; % deg, gap between segments - determines SIZE of rect, will be centered on a known point, so will be gap/2 at top/bottom/left/right too
p.bar_extent  = 1;   % % of square the bar subtends/100 (NOTE: gap will be drawn at top/bottom or left/right of entire bar, too)
p.dot_density = 10*0.1 ; % per deg^2 (seems high...but from biorxiv... - 124 dots per 4 deg^2
p.dot_size_deg= 0.075*5; % deg

% assign segments (1..n_segments, left->right, top->bottom)
if p.n_segments == 1 % note: only option right now....could get fancier? distractors?
    p.target_segments = [1]; % seg 1 is top/left
    p.sample_segments = [1];
end
...
p.fix_color = [255 0 0] * 255; %was 0.75 on all three
p.fix_size  =  0.25*2; % radius, dva
...
%only need to genenerate dot directions for 1 segment -1 = left down or 1 =
%right up
p.segment_dirs(:,1) = round(rand(size(p.segment_dirs,1),1))*2-1;
...
%convert -1 and 1 to 1 and 2 for key mapping
p.corr_resp = (p.segment_dirs(:,1) == 1) + 1;
% TCS quickf ix
...
p.resp_keys = [KbName('LeftArrow'),KbName('RightArrow'),KbName('DownArrow'),KbName('UpArrow')]; 
...
for ss = 1:size(p.seq,1)
    ...
    for step_num = 1:p.n_steps
        ...
        % segment position
        if p.seq(ss,1) == 1 || p.seq(ss,1) == 2 % x is p.bar_segment_centers_deg, y is this_bar_pos(step_num)
        ...
        while (GetSecs - p.expt_start)  < (p.step_dur*trial_counter + p.start_wait)
            ...
            for bb = 1:p.n_segments
                ...
                if p.seq(ss,1)==1 || p.seq(ss,1) == 2  % bar is horizontal
            ...
            if resp ~= 0
                ...
                elseif resp_yet == 0
                    ...
                    % if (p.seq(ss,1)==3)||(p.seq(ss,1)==4)
                    % this_resp = mod(this_resp,2)+1;
                    % end

                    %covert 3 and 4 to 1 and 2 to compare compare against corr_resp
                    if (p.seq(ss,1)==3)||(p.seq(ss,1)==4)
                        this_resp = this_resp-2;
                    end
                    %only respond to valid keys    
                    if (this_resp == 1)|| (this_resp == 2)
        ...
        if trial_counter <= (p.n_steps * size(p.seq,1))
            ...
            tmpfn = sprintf('./data/%02.f_r%02.f_lastCoh.mat',p.subj,p.run);
            init_coh = next_coh;
            save(tmpfn,'init_coh');
